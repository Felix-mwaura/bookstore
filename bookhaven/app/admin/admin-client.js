...
function Avatar({ name, email, size = "sm" }) {
  ...
  const initials = cleanedName
    ? cleanedName.split(/\s+/).filter(Boolean).map(w => w[0]).slice(0, 2).join("").toUpperCase()
    : (email || "?")[0].toUpperCase();
...
function OverviewTab({ orders, users, booksCount, loading }) {
  // Exclude cancelled/failed orders from revenue computations
  const validOrders = useMemo(() => orders.filter(o => o.status !== "Cancelled" && o.status !== "Failed"), [orders]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#1C1917] to-[#2d1a10] rounded-2xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <p className="text-stone-400 text-xs font-bold uppercase tracking-widest mb-1">This Month Revenue</p>
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-5">
          ...
          <div className="space-y-3">
            {users.slice(0, 4).map((u, i) => {
              const spent = validOrders.filter(o => o.user_id === u.id).reduce((s, o) => s + (o.total || 0), 0);
              return (
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-stone-400 text-xs mt-0.5">{orders.length} total · KSh {orders.filter(o => o.status !== "Cancelled" && o.status !== "Failed").reduce((s, o) => s + (o.total || 0), 0).toLocaleString()} revenue</p>
        </div>
              ...
              <tbody className="divide-y divide-stone-50">
                {filtered.map(book => (
                  <tr key={book.id} className="hover:bg-stone-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-3">
                      <p className="text-xs font-bold text-[#1C1917]">KSh {(book.price || 0).toLocaleString()}</p>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-semibold text-stone-600">{book.rating || "0.0"}</span>
...
function CustomersTab({ users, orders }) {
  ...
  const { ordersByUser, avgLtv } = useMemo(() => {
    ...
    return {
      ordersByUser: map,
      avgLtv: activeUsers > 0 ? Math.round(totalLtvSum / activeUsers) : 0,
    };
  }, [users, orders]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        ...
        <div className="bg-white rounded-2xl border border-stone-200 p-4 text-center">
          <p className="text-2xl font-black text-green-600">{users.filter(u => (ordersByUser[u.id]?.count || 0) > 0).length}</p>
          <p className="text-xs text-stone-400 mt-0.5">Have ordered</p>
        </div>
        <div className="bg-white rounded-2xl border border-stone-200 p-4 text-center">
          <p className="text-2xl font-black text-[#991B1B]">KSh {avgLtv.toLocaleString()}</p>
      ...
      <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-stone-50">
              {filtered.map(u => {
                const userOrderStats = ordersByUser[u.id] || { count: 0, total: 0 };
                return (
                  <tr key={u.id} className="hover:bg-stone-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-[#1C1917]">{userOrderStats.count}</td>
                    <td className="px-4 py-3 text-xs font-bold text-green-600">{userOrderStats.total > 0 ? `KSh ${userOrderStats.total.toLocaleString()}` : "—"}</td>
              ...
              {filtered.map(u => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-stone-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-bold text-[#1C1917]">{stats.count}</td>
                    <td className="px-4 py-3 text-xs font-bold text-green-600">{stats.total > 0 ? `KSh ${stats.total.toLocaleString()}` : "—"}</td>
                    <td className="px-4 py-3 text-xs text-stone-400 whitespace-nowrap">{new Date(u.created_at).toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "2-digit" })}</td>
                    <td className="px-4 py-3">
                      {!isYou && (
